...
from sqlalchemy import Column, ForeignKey, Integer, String, Date, Float, Enum, ChoiceType
...
#defining constants
FLOW_DIRECTION=enum.Enum('FLOW_DIRECTION','Credit Debit')
ACCOUNT_TYPE=enum.Enum('ACCOUNT_TYPE','Balanco Gestao Especiais')
#Class setup
...
class AccountGroup(Base):
	...
	id = Column(Integer, primary_key=True)
	name = Column(String(80), nullable=False)
	type= Column(ChoiceType(ACCOUNT_TYPE), nullable=False)
...
class Flow(Base):
	...
	direction = Column( ChoiceType(FLOW_DIRECTION), nullable=False)
