...
--SELECT player_names.player_id AS ID,
--player_names.player_name AS Name,
--COALESCE(COUNT(matches.id_looser),0) AS Record
--FROM player_names LEFT JOIN matches
...
--SELECT player_names.player_id AS ID,
--player_names.player_name AS Name,COUNT(matches.match_id) AS Played
--FROM player LEFT JOIN matches
--ON player_names.player_id = matches.winner_id or player.id = matches.loser_id
--GROUP BY player_names.player_id;
...
--COALESCE(matches_count.Record,'0'),
--matches_count.Played
--FROM matches_count LEFT JOIN win_count 
--ON win_count.ID = countMatches.ID 
--GROUP BY ID DESC;